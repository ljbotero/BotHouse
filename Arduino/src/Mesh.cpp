...
bool ICACHE_FLASH_ATTR tryConnectingToBetterAccessPoint(
  ...
  // Then, try wifi second
  if (accessPointHomeWifi != nullptr && strongestAccessPoint == nullptr) {
    wifiInfo = AccessPoints::getAccessPointInfo(flashData.wifiName);
    if (wifiInfo != nullptr &&
  ...
  if (strongestAccessPoint == nullptr) {
    ...
    if (strongestAccessPoint != nullptr) {
      Logs::serialPrintln(me, PSTR("Found last resource AP to connect to: "),
          String(strongestAccessPoint->SSID).c_str());
    }
  }
...
void ICACHE_FLASH_ATTR postScanNetworkAnalysis(Storage::storageStruct &flashData) {
  ...
  if (tryConnectingToBetterAccessPoint(flashData, accessPointHomeWifi)) {
    forceScan = true;
  }
...
void ICACHE_FLASH_ATTR scanNetworksComplete(int numberOfNetworks) {
  if (_scanNetworksCounter == 0) {
    ...
    AccessPoints::setAccessPointHomeWifi(NULL);
  }
  Storage::storageStruct flashData = Storage::readFlash();
